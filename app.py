...
from wtforms import Form, StringField, PasswordField, TextAreaField, IntegerField, validators, EmailField
...
class SignUpForm(Form):
    first_name = StringField('First Name', [validators.Length(min=1, max=100),Regexp('^[A-Za-z]+$', message="First name must contain only alphabetic characters")])
    last_name = StringField('Last Name', [validators.Length(min=1, max=100),Regexp('^[A-Za-z]+$', message="Last name must contain only alphabetic characters")])
    email = EmailField(
    'Email address',
    [
    ...
    username = StringField('Username', [validators.Length(min=4, max=100),Regexp(
            r'.*\d.*',
            message='Username must contain at least one numeric value'
        )])
    password = PasswordField(
    'Password',
    [
        validators.DataRequired(),
        validators.Length(min=8, max=15, message='Password must be between 8 and 15 characters long'),
        Regexp(
            r'(?=.*\d)(?=.*[\W_])',
            message='Password must contain at least one numeric value and one special character'
            ),
